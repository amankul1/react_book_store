        ...
        if (isLastNameValid && isFirstNameValid && emailValid && passwordValid) {
            try {
                const response = await axios({
                    method: 'post',
                    url: 'http://pj-bookstore.herokuapp.com/register/user',
                    data: {
                        'email': email,
                        'name': firstName,
                        'occupation': role,
                ...
                <div className={css.formWrapper}>
                    <form className="form-control">
                        ...
                        <div className="form-control">
                            ...
                            <input
                                style={(lastNameDirty && isLastNameValid) ? {borderColor: "#732C9F"} : {borderColor: "red"}}
                                className={css.formInput} type="text" id="lastName" onBlur={() => setLastNameDirty(true)}
                                onChange={lastNameHandler} value={lastName}/>
                        </div>
                        <br/>
                        <div className="form-control">
                            <label className={css.formLabel} htmlFor="firstName">First name</label><br/>
                            <input
                                style={(firstNameDirty && isFirstNameValid) ? {borderColor: "#732C9F"} : {borderColor: "red"}}
                                className={css.formInput} type="text" id="firstName" onBlur={() => setFirstNameDirty(true)}
                                onChange={firstNameHandler} value={firstName}/>
                        </div>
                        <br/>
                        <div className="form-control">
                            <label className={css.formLabel} htmlFor="email">Email</label><br/>
                            <input style={(emailDirty && emailValid) ? {borderColor: "#732C9F"} : {borderColor: "red"}}
                                   className={css.formInput} type="text" id="email" onBlur={() => setEmailDirty(true)}
                                   onChange={emailHandler} value={email}/>
                            {isEmailVerify ? <span style={{color:"red"}}>You already have an account </span>:null}
                        </div>
                        <br/>
                        <div className="form-control">
                        ...
                        <div className={css.formButton}>
                            <button style={{backgroundColor: "#732C9F", color: "white", width: "80%"}} onClick={sendHandler}
                                    className="btn">Send
                            </button>
                        </div>
                    </form>
